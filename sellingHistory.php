...
<head>
    ...
    <title>Selling History | Admins | BuyNow</title>
            ...
            <div class="col-12 bg-dark mt-3 mb-3">
                <div class="row">
                    <div class="col-12 col-lg-3 mt-3 mb-3">
                        <label class="form-label text-white fs-5">Search by Invoice ID : </label>
                        <input type="text" class="form-control fs-5" placeholder="Enter Invoice ID..." id="searchtxt" onkeyup="searchInvoiceId();" />
                    </div>
                    <div class="col-12 col-lg-2 mt-3 mb-3"></div>
                    <div class="col-12 col-lg-3 mt-3 mb-3">
                        <label class="form-label text-white fs-5">From Date : </label>
                        <input type="date" class="form-control fs-5" id="from" />
                    </div>
                    <div class="col-12 col-lg-3 mt-3 mb-3">
                        <label class="form-label text-white fs-5">To Date : </label>
            ...
            <div class="col-12 mt-2" id="viewArea">
                ...
                ?>
                    <div class="row">
                        ...
                        <div class="col-1 bg-dark text-end">
                            <label class="form-label fs-5 fw-bold text-white mt-1 mb-1"><?php echo $selected_data["payment_id"]; ?></label>
                        </div>
                        <?php
                        $product_rs = Database::search("SELECT * FROM `products` WHERE `id`='" . $selected_data["products_id"] . "'");
                        ...
                        <div class="col-2 bg-white d-grid">
                            ...
                            ?>
                                <button class="btn btn-success fw-bold mt-1 mb-1" onclick="changeInvoiceStatus('<?php echo $selected_data['payment_id']; ?>');" id="btn<?php echo $selected_data["payment_id"]; ?>">Confirm Order</button>
                            <?php
                            } else if ($selected_data["status"] == 1) {
                            ?>
                                <button class="btn btn-warning fw-bold mt-1 mb-1" onclick="changeInvoiceStatus('<?php echo $selected_data['payment_id']; ?>');" id="btn<?php echo $selected_data["payment_id"]; ?>">Packing</button>
                            <?php
                            } else if ($selected_data["status"] == 2) {
                            ?>
                                <button class="btn btn-info fw-bold mt-1 mb-1" onclick="changeInvoiceStatus('<?php echo $selected_data['payment_id']; ?>');" id="btn<?php echo $selected_data["payment_id"]; ?>">Dispatch</button>
                            <?php
                            } else if ($selected_data["status"] == 3) {
                            ?>
                                <button class="btn btn-primary fw-bold mt-1 mb-1" onclick="changeInvoiceStatus('<?php echo $selected_data['payment_id']; ?>');" id="btn<?php echo $selected_data["payment_id"]; ?>">Shipping</button>
                            <?php
                            } else if ($selected_data["status"] == 4) {
                            ?>
                                <button class="btn btn-danger fw-bold mt-1 mb-1 disabled" onclick="changeInvoiceStatus('<?php echo $selected_data['payment_id']; ?>');" id="btn<?php echo $selected_data["payment_id"]; ?>">Delivered</button>
                            <?php
                            }
                            ?>
