...
if(isset($_SESSION["u"])){
    ...
    if(isset($_FILES["image"])){
        ...
        if(!in_array($file_ex,$allowed_image_extentions)){
            ...
            if($image_num == 1){
                Database::iud("UPDATE `profile_image` SET `img_path`='".$file_name."' WHERE
                `user_email`='".$_SESSION["u"]["email"]."'");
            }else{
                Database::iud("INSERT INTO `profile_image`(`img_path`,`user_email`) VALUES
    ...
    if ($address_num == 1) {
        ...
        Database::iud("UPDATE `user_has_city` SET `line1`='" . $line1 . "',
                `line2`='" . $line2 . "',
                `city_id`='" . $city . "',
                `postal_code`='" . $pcode . "' WHERE `user_email`='" . $_SESSION["u"]["email"] . "'");
    } else {

        Database::iud("INSERT INTO `user_has_city` 
